...
async def test_create_task(task_data, user_data, async_client):
    request_user = await async_client.post(
    ...
    request_task = await async_client.post(
        f"http://127.0.0.1:8000/tasks/{user_id}/", json=task_data
    )
...
async def test_complete_and_delete_task(task_data, async_client, user_data):
    ...
    request_complete_task = await async_client.put(
        f"http://127.0.0.1:8000/tasks/{user_id}/{task_id}/complete/"
    )
    ...
    request_delete_task = await async_client.delete(
        f"http://127.0.0.1:8000/tasks/{user_id}/{task_id}/"
    )
...
async def test_delete_all_tasks(task_data, user_data, async_client):
    ...
    for count_tasks in range(random.randint(2, 5)):
        ...
        assert request_task.status_code == status.HTTP_200_OK
    request_delete_all_tasks = await async_client.delete(
        f"http://127.0.0.1:8000/tasks/{user_id}/"
    )
