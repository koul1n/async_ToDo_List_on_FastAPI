...
async def test_create_task(async_client, user_data, task_data):
    ...
    task_request = await async_client.post(f"http://127.0.0.1:8000/tasks/me/", json=task_data, headers=headers)
...
async def test_get_task(async_client, user_data, task_data):
    ...
    task_get_request = await async_client.get(f"http://127.0.0.1:8000/tasks/me/", headers=headers)
...
async def test_delete_all_tasks(async_client, user_data, task_data):
    ...
    task_request = await async_client.post(f"http://127.0.0.1:8000/tasks/me/", json=task_data, headers=headers)
    assert task_request.status_code == status.HTTP_200_OK
    task_delete_request = await async_client.delete(f"http://127.0.0.1:8000/tasks/me/", headers=headers)
...
async def test_delete_task(async_client, user_data, task_data):
    ...
    task_delete_request = await async_client.delete(f"http://127.0.0.1:8000/tasks/me/{task_id}/", headers=headers)
    assert task_delete_request.status_code == status.HTTP_200_OK
...
async def test_complete_task(async_client, user_data, task_data):
    ...
    assert task_request.status_code == status.HTTP_200_OK
    task_id = task_request.json()['id']
    task_complete_request = await async_client.put(f"http://127.0.0.1:8000/tasks/me/{task_id}/complete/", headers=headers)
