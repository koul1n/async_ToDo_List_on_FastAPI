...
async def test_get_user(async_client, user_data):
    ...
    get_request = await async_client.get(f"http://127.0.0.1:8000/users/{user_id}/", headers=headers)
...
async def test_delete_user(async_client, user_data):
    ...
    token = response_json['access_token']
    headers = {"Authorization": f"Bearer {token}"}
    delete_request = await async_client.delete(f"http://127.0.0.1:8000/users/{user_id}/", headers=headers)
...
async def test_update_user(async_client, user_data):
    ...
    update_request = await async_client.patch(f"http://127.0.0.1:8000/users/update/", headers=headers, json=data)
